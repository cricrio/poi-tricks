...
export function CreatorGroup({ creators }: Props) {
  return (
    <div className="flex items-center gap-4">
      <AvatarGroup>
        {creators?.map((creator) => <Avatar key={creator.id} {...creator} />)}
      </AvatarGroup>
      <div>
        {creators?.map((creator, index) => (
          <div key={creator.id}>
            <Link to={ROUTES.creator(creator)} className=" normal-case">
              {creator.name}
            </Link>
            {index < creators.length - 1 && <span className="mr-1">,</span>}
          </div>
        ))}
      </div>
    </div>
